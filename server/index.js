...
const briaRmbgService = briaRmbgConfigured
  ? createBriaRmbgService({
      ...
      onCacheError: (error) => console.error(`[bria-rmbg] cache_write_failed code=${error?.code || 'FS_ERROR'}`),
    })
  : null;
if (briaRmbgService) {
  await briaRmbgService.ready().catch((error) => {
    console.error(`[bria-rmbg] disabled code=${error?.code || 'CACHE_UNAVAILABLE'}`);
  });
}
const isBriaRmbgReady = () => Boolean(briaRmbgService?.isReady());
const briaBodyAdmission = createBriaBodyAdmission(briaConcurrency);
const limitBriaRequests = createBriaRequestRateLimiter({
  limit: boundedPositiveInteger(process.env.BRIA_RMBG_RATE_LIMIT, 20, 100),
  windowMs: 10 * 60 * 1000,
  maxBuckets: 1_000,
});
...
// Keep this billable upload outside the legacy 150 MiB parser. A 10 MiB image
// expands to roughly 13.4 MiB as base64, leaving bounded room for JSON syntax.
app.use(
