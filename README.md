...
**Cover** (ранее Fusion AI) — React-приложение для создания обложек методом слияния нескольких изображений персонажей в единую сцену. Работает на базе Google Gemini API и разработано для запуска в среде [Google AI Studio](https://aistudio.google.com).
...
`GEMINI_API_KEY` хранится только в окружении Express-сервиса. Браузерный SDK
работает через same-origin `/api/gemini`; ключ не встраивается в JS-бандл.

### Модели Gemini

| Модель                           | Применение              | imageSize | Особенности                    |
|----------------------------------|-------------------------|-----------|--------------------------------|
| `gemini-2.5-flash-image`         | Создание / Апскейл      | фикс. 1K  | Самая быстрая                  |
| `gemini-3.1-flash-image`         | Создание / Апскейл      | 512px–4K | Баланс скорости и качества     |
| `gemini-3-pro-image`             | Создание / Апскейл      | 1K–4K    | Максимальное качество          |

> `imageSize` отправляется только для `gemini-3.1-flash-image` и `gemini-3-pro-image`.
> Для 2.5 доступен фиксированный режим 1K; для 3 Pro настройка `512px` отключена.
> Неподдерживаемые форматы и размеры нормализуются повторно перед запросом.
...
| Ключ провайдера | `GEMINI_API_KEY` остаётся в server-only окружении; браузер использует same-origin proxy |
